...
const HomeCreator = () => {
  ...
  const { data: totalPayment = [], isLoading: load } = useQuery({
    ...
    queryFn: async () => {
      const response = await axiosSecure.get(`totalPayment/${user?.email}`);
      return response.data;
    }
  });

  const totalPaymentsum = totalPayment?.reduce((prevValue, value) => prevValue + value, 0);
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          updateStatus(title, id, "approved", workerEmail, upCoin);
        }
      });
    } catch (err) {
    ...
    <div className=" px-4 py-8">
      ...
      <div className="rounded-lg border border-gray-200 max-h-96 overflow-y-auto">
        <div className="table-container">
          <table className="table table-xs bg-white border-collapse w-full">
              ...
              {allPendingData.map((data, idx) => (
                <tr key={data._id} className={idx % 2 === 0 ? 'bg-gray-100' : 'bg-white'}>
                  ...
                  <td className="px-4 py-2 border text-center">
                    <button
                      className="bg-[#0bb990] rounded py-2 px-4 text-white flex items-center justify-center w-32"
                      onClick={() => handleViewSubmission(data)}
                    >
                      <AiOutlineEye className="mr-2" /> View
                    </button>
                  </td>
                  <td className="px-4 py-2 border text-center justify-center">
                    <button
                      className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-l flex items-center w-32"
                      onClick={() => handleApprove(data.task_title, data._id, data.worker_email, data.payable_amount)}
                    >
                    ...
                    <button
                      ...
                      onClick={() => handleReject(data.task_title, data._id, data.worker_email)}
                    >
          ...
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              ...
              <div className="mt-5 sm:mt-6">
                <button
                  className="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#0bb990] sm:text-sm"
